...
app = FastAPI(
    ...
    description="Production-grade, memory-optimized financial document processing engine.",
    version="5.0.0"
)

# Global Initialization - Hooks safely into Render's Environment Variable panel
...
def clean_currency(val: Optional[str]) -> Optional[float]:
    """Converts accounting bookkeeping numbers safely into clean float data structures."""
    if not val: 
        return None
    try:
...
def process_scanned_pdf_via_ocr(pdf_bytes: bytes) -> str:
    """Lightweight text extractor that handles scan overlays within 60MB of RAM."""
    flattened_text = []
...
def llm_fallback_w2(text_context: str) -> W2TaxData:
    """Failsafe schema layout alignment via structured Gemini model extractions."""
    if not ai_client: 
        return W2TaxData()
...
def llm_fallback_sec(text_context: str) -> List[SECBalanceSheetRow]:
    """Failsafe layout matrix structural table extraction layer."""
    if not ai_client: 
        return []
